...
- **Logo backdrop (approved 2026-06-03):** A single NP mark sits fixed behind all content as a faint watermark (`.logo-backdrop`, a real element in `LogoBackdrop.tsx`, `9%` opacity, `center 38%`, `min(82vw, 820px)`). It **drifts upward on scroll** at half the scroll speed (see Motion → Logo parallax) so the mark clears out from behind body copy as the reader descends. Stacking is load-bearing: paper lives on `html` (the canvas), `body` has **no** background, and the backdrop sits at `z-index: -1` — so it paints above the paper canvas but below content. A background on `body` (or `bg-paper` on `<main>`) would paint over it and hide it, so both `<main>` elements are transparent. Asset: `public/np_logo.svg` (ink stroke). This is the one decorative graphic on the site.
...
- **Logo parallax (approved deviation, 2026-06-03):** The logo backdrop drifts upward at 0.5× scroll speed (`translate3d` via a passive scroll listener + rAF, transform-only). This crosses the brief's "no parallax" rule by explicit user request — its purpose is legibility, not spectacle: the static mark sat behind body copy and competed with it, so moving it out of the way as you scroll keeps text readable. Scope is limited to the single decorative backdrop. Disabled under `prefers-reduced-motion` (and without JS), where the mark stays static.
...
- **Forbidden by brief:** Animated counters, hero video, scroll-driven choreography, entrance animations on sections *other than the Work-card entrance above*. (Parallax is forbidden except the single approved logo-backdrop drift above.)
...
| 2026-06-03 | Logo backdrop drifts up on scroll (parallax) | The static centered mark competed with body copy ("hard to read text on top of the logo"). User asked to make it slowly scroll up. Implemented as a 0.5× transform-only parallax (LogoBackdrop.tsx), reduced-motion + no-JS safe. Crosses the brief's no-parallax rule by explicit request; scope limited to the one decorative backdrop, justified by legibility. Opacity also eased 10%→9%. |
